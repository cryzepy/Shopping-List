...
function data_server(action) {
  let req = new XMLHttpRequest();

  // utility
  const X_MASTER_KEY = localStorage.getItem("x-master-key-from-shopping-list");
  const BIN_ID = localStorage.getItem("bin-id-from-shopping-list");

  console.log(X_MASTER_KEY);
  console.log(BIN_ID);

  req.open(action, `https://api.jsonbin.io/v3/b/${BIN_ID}`, true);
  ...
  switch (action) {
    case "PUT":
      ...
      req.send(JSON.stringify(local_database({ type: "GET" })));
      break;
    case "GET":
      req.onreadystatechange = () => {
        if (req.readyState == XMLHttpRequest.DONE) {
      ...
      for (let i = 0; i < data.length; i++) {
        if (data[i].id == props.payload.id) {
          data[i] = {
            ...
            priority:
              props.payload.priority === undefined
                ? data[i].priority
...
function element_builder(props) {
  ...
  if (props.attributes)
    Object.entries(props.attributes).forEach(
      (item) => item[1] && element.setAttribute(item[0], item[1])
    );
...
function handle_duplicate_input(props) {
  ...
  for (let index = 0; index < data.length; index++) {
    if (
      data[index].item_name.toLowerCase() == props.payload.name.toLowerCase()
    ) {
...
function add_list_el() {
  ...
  sort_data_by(order_by({ type: "GET" }))
      ...
      const i3 = element_builder({
        ...
        attributes: {
          class: item.priority
            ...
            : "bi-file-earmark-plus" + " btn",
        },
      });

      i3.className.includes("minus")
...
function add_total_item_el() {
  ...
  element_builder({
    ...
    innerText:
      length == 0 ? "No List, Click The + Icon to Add" : `${length} items`,
  });
}
...
function add_total_price_el() {
  const prices = price_all_items(
    local_database({ type: "GET" }).map((price) => +price.item_price)
  );
...
function add_sort_by_el() {
  ...
  const label = sort.includes("name")
    ? "Name"
    : sort.includes("price")
    ? "Price"
    : "Time";
...
function add_input_item_el(props) {
  ...
  input_item.addEventListener(
    "keydown",
    (e) => e.key == "Enter" && input_price.focus()
  );
  ...
  clickEvent(button, () => {
    const priceCond =
      input_price.value.trim() && +input_price.value >= 0 ? true : false;
    const nameCond = input_item.value.trim() ? true : false;

    function handleSuccess() {
      if (
        handle_duplicate_input({
          action: props.action,
          payload: { name: input_item.value.trim(), id: props.id },
        })
      ) {
    ...
    function handleFailed() {
      nameCond
        ? input_item.classList.remove("invalid")
        : input_item.classList.add("invalid");
      priceCond
        ? input_price.classList.remove("invalid")
        : input_price.classList.add("invalid");
    }
...
function add_setting_el() {
  ...
  const [del, set, get, insert] = add_popup_el([
  ...
  get &&
    ...
    insert && clickEvent(insert, () => {
        window.location.href = "insert-code.html"
    })
}
